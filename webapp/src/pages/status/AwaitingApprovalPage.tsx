  ...
  return (
    <div className="min-h-screen flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 relative text-slate-900 dark:text-white">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        <div className="bg-white dark:bg-[#121A15]/90 py-8 px-6 shadow-xl dark:shadow-2xl rounded-3xl sm:px-10 border border-emerald-950/10 dark:border-white/10 text-center backdrop-blur-xl">
          <div className="w-20 h-20 rounded-full bg-amber-500/15 text-amber-600 dark:text-amber-400 flex items-center justify-center mx-auto mb-4 border border-amber-400/30">
            <Clock className="w-10 h-10 animate-pulse" />
          </div>

          <h2 className="text-2xl font-extrabold text-slate-900 dark:text-white">{t('pendingApprovalTitle')}</h2>
          <p className="text-sm font-medium text-slate-600 dark:text-slate-300 mt-2 leading-relaxed">
            {t('pendingApprovalMsg')}
          </p>

          {message && (
            <div className="mt-4 p-3 rounded-xl bg-amber-500/15 border border-amber-400/30 text-xs font-bold text-amber-800 dark:text-amber-300">
              {message}
            </div>
          )}
          ...
          <div className="text-left bg-slate-50 dark:bg-[#0B130E] rounded-2xl p-4 border border-slate-200 dark:border-white/10 space-y-2 mb-6">
            ...
            <div className="flex justify-between text-xs font-semibold">
              ...
              <span className="text-slate-900 dark:text-white">+91 {user?.phone || '-'}</span>
            </div>
            <div className="flex justify-between text-xs font-semibold">
          ...
          <button
            ...
            disabled={refreshing}
            className="w-full py-3.5 px-4 bg-[#1E4620] hover:bg-[#163318] text-white font-bold text-sm rounded-xl shadow-md transition-all active:scale-[0.99] disabled:opacity-70 flex items-center justify-center space-x-2"
          >
