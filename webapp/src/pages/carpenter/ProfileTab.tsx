...
export const ProfileTab: React.FC<ProfileTabProps> = ({
  ...
  return (
    <div className="space-y-6 pb-28 sm:pb-12 max-w-4xl mx-auto text-slate-900 dark:text-white">
      {/* 1. User Summary Profile Card (Thin Green Box with White Dotted Line) */}
        ...
        <div className="relative z-10 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center space-x-4">
              ...
              <div className="w-16 h-16 rounded-full bg-gradient-to-br from-[#1E4620] via-[#10B981] to-[#0A160D] flex items-center justify-center text-white font-black text-2xl shadow-md border-2 border-white ring-2 ring-[#10B981]/50 flex-shrink-0">
                {(user.name || 'R').charAt(0).toUpperCase()}
              </div>
              <div>
                <div className="flex items-center space-x-2">
                  <h3 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight">{user.name || 'Carpenter'}</h3>
                  {stats.verified && (
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 border border-emerald-400/40">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-4 border-t border-slate-100 dark:border-white/10 text-xs">
            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#0B130E] border border-[#10B981]/30 transition-colors">
              <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Loyalty Tier</span>
              <p className="text-sm font-black text-amber-700 dark:text-amber-300 mt-0.5 flex items-center space-x-1">
            ...
            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#0B130E] border border-[#10B981]/30 transition-colors">
              ...
              <p className="text-sm font-black text-slate-900 dark:text-white mt-0.5">{stats.totalSheets || 0} Sheets</p>
            </div>
            <div className="col-span-2 sm:col-span-1 p-3.5 rounded-xl bg-slate-50 dark:bg-[#0B130E] border border-[#10B981]/30 transition-colors">
              <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Cashback Rate</span>
              <p className="text-sm font-black text-emerald-700 dark:text-emerald-400 mt-0.5">{stats.tierRewardPct || 0.8}% per sheet</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {isEditing ? (
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="pt-4 flex items-center justify-end space-x-3">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="btn-primary-amber px-7 py-3 text-xs uppercase tracking-wider font-black disabled:opacity-50 border-2 border-white/40"
                >
        ...
        /* Read-Only View */
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {/* Personal Details (Thin Green Box with White Dotted Line) */}
          <div className="relative rounded-2xl bg-white/90 dark:bg-[#121A15]/90 backdrop-blur-xl border border-[#10B981]/40 p-6 shadow-xl shadow-emerald-900/5 space-y-4 transition-all duration-300 before:absolute before:inset-1.5 before:border before:border-dashed before:border-white/80 dark:before:border-white/20 before:pointer-events-none before:rounded-xl">
            <div className="relative z-10 space-y-4">
              ...
              <div className="space-y-3 text-xs divide-y divide-slate-100 dark:divide-white/10">
                ...
                <div className="pt-2 flex justify-between">
                  ...
                  <span className="font-semibold text-slate-900 dark:text-white">{user.region || 'Hubballi, Karnataka'}</span>
                </div>
                <div className="pt-2 flex justify-between">
          ...
          {/* Bank & UPI Details (Thin Green Box with White Dotted Line) */}
          <div className="relative rounded-2xl bg-white/90 dark:bg-[#121A15]/90 backdrop-blur-xl border border-[#10B981]/40 p-6 shadow-xl shadow-emerald-900/5 space-y-4 transition-all duration-300 before:absolute before:inset-1.5 before:border before:border-dashed before:border-white/80 dark:before:border-white/20 before:pointer-events-none before:rounded-xl">
            <div className="relative z-10 space-y-4">
              ...
              <div className="space-y-3 text-xs divide-y divide-slate-100 dark:divide-white/10">
                ...
                <div className="pt-2 flex justify-between">
                  ...
                  <span className="font-semibold text-slate-900 dark:text-white">{user.bank_name || 'Not Set'}</span>
                </div>
                <div className="pt-2 flex justify-between">
