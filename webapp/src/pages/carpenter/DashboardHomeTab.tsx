...
import { Camera, CheckCircle2, Clock, FileText, ChevronRight, Gift, Trophy, ArrowUpRight, Sparkles, ShieldCheck, RefreshCw } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 pb-28 sm:pb-12 max-w-4xl mx-auto">
      ...
      <motion.div
        ...
        className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 sm:p-6 rounded-2xl bg-[#121A15]/80 backdrop-blur-xl border border-white/10 shadow-xl"
      >
        <div className="flex items-center space-x-4">
          <div className="w-13 h-13 rounded-2xl bg-gradient-to-br from-[#1E4620] to-[#0A160D] flex items-center justify-center text-white font-extrabold text-xl shadow-lg border border-emerald-500/30">
        ...
        <div className="self-start sm:self-auto">
          {stats.verified ? (
            <div className="inline-flex items-center px-3.5 py-1.5 rounded-full text-xs font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-400/40 shadow-glow-emerald">
              <ShieldCheck className="w-4 h-4 mr-1.5 text-emerald-400" />
              <span>Verified Account</span>
            </div>
          ) : (
            <div className="inline-flex items-center px-3.5 py-1.5 rounded-full text-xs font-bold bg-amber-500/20 text-amber-300 border border-amber-400/30">
      ...
      {/* 2. 3D Holographic "Perillo Rewards Pass" (Dynamic 4-Tier Background) */}
      <PerilloRewardsCard
        userName={user.name}
        userId={user.id}
      ...
      <div className="p-6 rounded-2xl bg-[#121A15]/80 backdrop-blur-xl border border-white/10 shadow-xl space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-300">Cumulative verified plywood sheets</p>
            </div>
          </div>
        ...
        {/* Milestone Banner */}
        {nextMilestone && (
          <div className="p-3.5 rounded-xl bg-amber-500/10 border border-amber-400/30 text-xs font-semibold text-amber-200 flex items-center justify-between shadow-glow-amber">
            <div className="flex items-center space-x-2.5">
              <Gift className="w-4 h-4 text-amber-400 flex-shrink-0 animate-pulse" />
              <span>
                🎁 Next gift at <strong>{nextMilestone.sheets} Sheets</strong>: {nextMilestone.gift}
              </span>
            </div>
            <span className="text-[10px] font-bold text-amber-300 uppercase tracking-wider bg-amber-500/20 px-2.5 py-1 rounded-lg border border-amber-400/30">
      ...
      <div className="grid grid-cols-3 gap-3.5 sm:gap-4">
        {/* Pending Card */}
        <div className="p-5 rounded-2xl bg-[#121A15]/80 backdrop-blur-xl border border-white/10 shadow-xl transition-transform hover:scale-[1.02]">
          <div className="flex items-center justify-between">
          ...
          <p className="text-[11px] text-slate-400 mt-1 leading-relaxed">Under Verification</p>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-[#121A15]/80 backdrop-blur-xl border border-white/10 shadow-xl transition-transform hover:scale-[1.02]">
          ...
          <p className="text-[11px] text-slate-400 mt-1 leading-relaxed">Points Credited</p>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-[#121A15]/80 backdrop-blur-xl border border-white/10 shadow-xl transition-transform hover:scale-[1.02]">
          ...
          <h4 className="text-2xl sm:text-3xl font-extrabold text-[#FFFFFF] mt-2">
            <CountUp to={stats.totalSheets || 0} />
          </h4>
          <p className="text-[11px] text-slate-400 mt-1 leading-relaxed">Lifetime Verified</p>
        </div>
      </div>
      ...
      <div className="space-y-3.5">
        ...
        {loading ? (
          <div className="text-center py-12 bg-[#121A15]/60 rounded-2xl border border-white/10 text-slate-400 text-xs">
            Loading recent invoices...
          </div>
        ) : recentClaims.length === 0 ? (
          <div className="p-8 text-center space-y-3 rounded-2xl bg-[#121A15]/80 backdrop-blur-xl border border-white/10 shadow-xl">
            ...
            <p className="text-xs text-slate-300 max-w-xs mx-auto leading-relaxed">{t('uploadFirst')}</p>
            <button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            {recentClaims.map((item) => (
              <div
                ...
                className="p-5 rounded-2xl bg-[#121A15]/80 backdrop-blur-xl border border-white/10 shadow-xl cursor-pointer group press-scale hover:border-emerald-500/40 transition-all"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3.5">
                    <div className="w-11 h-11 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-slate-400 group-hover:text-emerald-400 transition-colors flex-shrink-0">
