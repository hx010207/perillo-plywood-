...
export const CarpenterNavBar: React.FC<CarpenterNavBarProps> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-30 bg-[#0A160D]/90 backdrop-blur-2xl border-t border-white/10 shadow-2xl sm:sticky sm:bottom-auto sm:top-[68px] sm:border-t-0 sm:border-b sm:border-white/10">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`relative flex flex-col sm:flex-row items-center space-y-1 sm:space-y-0 sm:space-x-2 py-2 sm:py-3.5 px-3 sm:px-4 rounded-xl font-black text-xs sm:text-sm transition-colors ${
                  isActive
                    ? 'text-emerald-300'
              ...
              >
                ...
                {isActive && (
                  <motion.div
                    ...
                    className="absolute inset-0 bg-emerald-500/15 sm:bg-emerald-500/10 rounded-xl sm:rounded-none sm:border-b-2 sm:border-emerald-400 -z-10"
